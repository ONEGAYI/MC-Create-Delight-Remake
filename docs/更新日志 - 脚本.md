...
### Lightmans Currency 任务联动修复
修复FTB任务完成后无法解锁Lightmans Currency交易的问题。

**修复内容:**
- `server_scripts/Lightmans Currency/white_list.js`
  - 修正交易商ID获取方式：使用 `/lcadmin traderdata search` 命令获取的实际ID
  - 科技交易商ID: `7` → `10` (technology_help_trade)
  - 资源交易商ID: `10` → `14` (resource_trader)

**问题说明:**
Lightmans Currency的 `GetTrader()` API使用的是内部交易商ID，而非PersistentTraders.json中的数组索引。需通过游戏内命令 `/lcadmin traderdata search [交易关键词]` 获取正确的ID。
...
**新增脚本:**
- `server_scripts/SuiAn's scripts/recipes/Create Delight/diamond_convert.js`
  - 砂纸打磨配方：人工钻石 → 1个钻石 + 10%概率额外1个钻石
  - 研磨机配方：人工钻石 → 1个钻石 + 50%概率额外1个钻石

## v1.3.5

### 生物生成禁用扩展
新增对Alex's Mobs悬熊的生成禁用。

**改动内容:**
- `server_scripts/SuiAn's scripts/mob_control/alexsmobs_prevent_summon.js`
  - 新增悬熊（dropbear）生成禁用
  - 优化生物生成控制脚本结构
