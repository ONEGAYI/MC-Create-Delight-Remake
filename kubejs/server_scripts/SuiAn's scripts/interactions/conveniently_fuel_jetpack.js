...
// ==================== 顶部定义 (防止重复加载) ====================
const $JetpackCuriosApi = Java.loadClass("top.theillusivec4.curios.api.CuriosApi");

/**
 * 获取玩家拥有的喷气背包详细信息（包含槽位、NBT和物品对象）
 * @param {Player} player - 玩家
 * @returns {Object} 包含详细信息
 */
function getJetpackInfo(player) {
    ...
    const result = {
        netherite_jetpack: { itemId: "create_jetpack:netherite_jetpack", items: [] },
        jetpack: { itemId: "create_jetpack:jetpack", items: [] },
        totalCount: 0,
    };

    // 预先声明变量，防止在循环中被判"重复声明"
    let currentId, targetKey, location;

    // ---------------------------------------------------------
    // 1. 扫描饰品栏
    // ---------------------------------------------------------
    $JetpackCuriosApi.getCuriosInventory(player).ifPresent(handler => {
        const curiosMap = handler.getCurios();

        if (debugMode) {
            console.log(LOG_PREFIX + " [DEBUG] 开始扫描Curios饰品栏");
        }

        curiosMap.forEach((slotIdentifier, stacksHandler) => {
            const stackHandler = stacksHandler.getStacks();

            // 在循环外声明 curioStack
            let curioStack;

            for (let i = 0; i < stackHandler.getSlots(); i++) {
                const rawStack = stackHandler.getStackInSlot(i);

                if (rawStack && !rawStack.isEmpty()) {
                    // 使用局部变量避免污染
                    let rawId = rawStack.getItem().toString();
                    ...
                    for (var ti = 0; ti < targetIds.length; ti++) {
                        if (rawId === targetIds[ti] ||
                            rawId.indexOf("netherite_jetpack") >= 0 && targetIds[ti].indexOf("netherite_jetpack") >= 0 ||
                            rawId.indexOf(":jetpack") >= 0 && rawId.indexOf("netherite") < 0 && targetIds[ti] === "create_jetpack:jetpack") {
                            isTarget = true;
                            break;
                        }
                    }
                    ...
                    if (isTarget) {
                        curioStack = Item.of(rawStack); // 赋值，而不是 const 声明

                        targetKey = rawId.indexOf("netherite_jetpack") >= 0
                                          ? "netherite_jetpack" : "jetpack";

                        if (debugMode) {
                            console.log(LOG_PREFIX + " [DEBUG] 找到喷气背包: " + targetKey + ", 槽位: " + slotIdentifier + ", 索引: " + i);
                        }
        ...
        if (debugMode) {
            console.log(LOG_PREFIX + " [DEBUG] Curios扫描完成，总计: " + result.totalCount + "个");
        }
    });
    ...
    let invStack; // <--- 【关键修复】在循环外面声明变量！

    for (let i = 0; i < inventory.getContainerSize(); i++) {
        invStack = inventory.getItem(i); // <--- 【关键修复】这里只赋值，去掉 const/let
        ...
        if (invStack && !invStack.isEmpty()) {
            ...
            if (targetIds.indexOf(currentId) >= 0) {
                ...
                targetKey = currentId === "create_jetpack:netherite_jetpack"
                                  ? "netherite_jetpack" : "jetpack";
...
function getJetpackCapacity(jetpackItem) {
    ...
    if (nbt.Enchantments) {
        ...
        for (var i = 0; i < enchantments.length; i++) {
            ...
            if (debugMode) {
                console.log("[JetpackFuel] [DEBUG] 检查附魔: id=" + String(enchId) + ", lvl=" + ench.lvl);
            }

            // 尝试匹配多种可能的ID格式
            if (String(enchId).indexOf("capacity") >= 0 ||
                String(enchId).indexOf(" Capacity") >= 0 ||
                String(enchId) === "create_jetpack:capacity") {
                capacityLvl = Number(ench.lvl);
                if (debugMode) {
                    console.log("[JetpackFuel] [DEBUG] 找到扩容附魔，等级: " + capacityLvl);
                }
                break;
            }
        }
    ...
    if (debugMode) {
        console.log("[JetpackFuel] [DEBUG] 容量信息: 当前=" + air + ", 最大=" + maxAir + ", 扩容等级=" + capacityLvl);
    }
            ...
            for (var c = 0; c < curiosKeys.length; c++) {
                ...
                for (i = 0; i < stackHandler.getSlots(); i++) {
                    ...
                    if (rawStack && !rawStack.isEmpty()) {
                        ...
                        var isMatch = rawId.indexOf("netherite_jetpack") >= 0 && jetpackInfo.stack.id.indexOf("netherite_jetpack") >= 0 ||
                                       rawId.indexOf(":jetpack") >= 0 && rawId.indexOf("netherite") < 0 && jetpackInfo.stack.id.indexOf("jetpack") >= 0;

                        if (isMatch) {
                            // 找到目标物品，使用KubeJS方式更新NBT
                            if (debugMode) {
                                console.log("[JetpackFuel] [DEBUG] 找到目标，开始更新: 槽位=" + slotIdentifier + ", 索引=" + i + ", 新Air=" + newAir);
                            }
                            ...
                            if (debugMode) {
                                console.log("[JetpackFuel] [DEBUG] NBT修改: Air=" + currentNbt.Air);
                            }
                            ...
                            if (debugMode) {
                                console.log("[JetpackFuel] [DEBUG] setStackInSlot结果: " + result);
                            }
                            return;
                        }
                    }
...
BlockEvents.rightClicked("create:netherite_backtank", (event) => {
    ...
    if (debugMode) {
        ...
        console.log(`${LOG_PREFIX} 需要充能的喷气背包: ${needRefuelJetpacks.length}个`);
    }
    ...
    for (const jetpackData of needRefuelJetpacks) {
        ...
        if (transferAmount > 0) {
            ...
            if (debugMode) {
                const locName = jetpackData.location === "curios" ? "饰品" :
                                jetpackData.location === "hotbar" ? "快捷" : "背包";
                console.log(`${LOG_PREFIX} [${locName}] 充能: ${capacity.air} -> ${newJetpackAir} (转移${transferAmount})`);
            }
        }
    }
    ...
    server.runCommandSilent("setblock " + tankPos[0] + " " + tankPos[1] + " " + tankPos[2] + " " + block.id + tankState + tankData + " replace");
